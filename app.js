...
var _handCanvas = __webpack_require__(329);

var _handCanvas2 = _interopRequireDefault(_handCanvas);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

function _asyncToGenerator(fn) { return function () { var gen = fn.apply(this, arguments); return new Promise(function (resolve, reject) { function step(key, arg) { try { var info = gen[key](arg); var value = info.value; } catch (error) { reject(error); return; } if (info.done) { resolve(value); } else { return Promise.resolve(value).then(function (value) { step("next", value); }, function (err) { step("throw", err); }); } } return step("next"); }); }; }

var handController = void 0;
var handCanvas = void 0;

var standardHandSettings = {
  rColor: {
    r: 255,
    g: 15,
    b: 15,
...
function initHand(img) {
  ...
  // context to draw to
  var context = handCanvas.getContext('2d');
  // context.globalCompositeOperation = 'xor';

  handController = new _handCanvas2.default(context, handCanvas, img);
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.next = 2;
            return loadImage('client/images/hand.jpg');
...
function reachableViaDendrite(x1, y1, x2, y2, reachDist) {
  ...
  if (distance > reachDist || xDist === 0 && yDist === 0) {
    return false;
  }
  return true;
}
...
var Electron = function () {
  function Electron(settings, nodeArray, type, x, y, canvas, offscreenCanvas) {
    ...
    if (this.type === 'electron-standard') {
      var _settings$electronSta = settings.electronStandardSettings,
          lifeSpan = _settings$electronSta.lifeSpan,
          minRadius = _settings$electronSta.minRadius,
          maxRadius = _settings$electronSta.maxRadius;


      this.currentNode = nodeArray[Math.floor(Math.random() * nodeArray.length)];
      this.nextNode = this.currentNode.dendritesReachTowards[Math.floor(Math.random() * this.currentNode.dendritesReachTowards.length)];
      this.x = this.currentNode.x;
      this.y = this.currentNode.y;
      this.radius = Math.floor(Math.random() * maxRadius) + minRadius;
      this.lifeSpan = lifeSpan;
    } else {
      var _settings$electronSup = settings.electronSuperChargedSettings,
          _lifeSpan = _settings$electronSup.lifeSpan,
          radius = _settings$electronSup.radius;
  ...
  _createClass(Electron, [{
    ...
    value: function standardMovement() {
    ...
    key: 'superchargedOnHandMovement',
    value: function superchargedOnHandMovement(targetCanvas, offscreenCanvas) {
      if (typeof this.nextNode !== 'undefined') {
        var apsectRatio = offscreenCanvas.width / offscreenCanvas.height;

        var height = targetCanvas.height;

        var width = height * apsectRatio;

        var widthAdjustFactor = width / offscreenCanvas.width;
        var heightAdjustFactor = height / offscreenCanvas.height;

        var xOffset = targetCanvas.width / 2 - width / 2;
        var yOffset = targetCanvas.height / 2 - height / 2;

        var targetX = Math.floor(this.nextNode.x * widthAdjustFactor) + xOffset;
        var targetY = Math.floor(this.nextNode.y * heightAdjustFactor) + yOffset;
    ...
    key: 'findNearestNode',
    value: function findNearestNode(nodeArray, canvas, offscreenCanvas) {
      var _this = this;
      ...
      var widthAdjustFactor = offscreenCanvas.width / width;
      var heightAdjustFactor = offscreenCanvas.height / height;

      var xOffset = canvas.width / 2 - width / 2;
      var yOffset = canvas.height / 2 - height / 2;

      var adjustedX = Math.floor(this.x - xOffset) * widthAdjustFactor;
      var adjustedY = Math.floor(this.y - yOffset) * heightAdjustFactor;

      this.nearestDist = 999999;

      nodeArray.forEach(function (node) {
        var xDist = Math.abs(node.x - adjustedX);
        var yDist = Math.abs(node.y - adjustedY);

        if (Math.sqrt(xDist * xDist + yDist * yDist) < _this.nearestDist) {
          _this.nearestDist = Math.sqrt(xDist * xDist + yDist * yDist);
          _this.nearestNode = node;
        }
      });
    }
  }, {
    key: 'headTowardsNearestNode',
    value: function headTowardsNearestNode(targetCanvas, offscreenCanvas) {
      ...
      var targetX = Math.floor(this.nearestNode.x * widthAdjustFactor) + xOffset;
      var targetY = Math.floor(this.nearestNode.y * heightAdjustFactor) + yOffset;
      ...
      var closeEnoughX = Math.abs(targetX - this.x) < 7;
      var closeEnoughY = Math.abs(targetY - this.y) < 7;
    ...
    key: 'update',
    value: function update(targetCanvas, offscreenCanvas) {
      this.lifeSpan -= 1;
      if (this.lifeSpan < 1) {
        return 'dead';
      }
    ...
    key: 'drawSuperchargedTrial',
    value: function drawSuperchargedTrial(context, rColor) {
      if (this.isExploding === false) {
        var ctx = context;
        if (this.movement.length > 2) {
          for (var m = 0; m < this.movement.length; m += 2) {
            ctx.beginPath();
...
var Dendrite = function () {
  function Dendrite(settings, x0, y0, x1, y1) {
    _classCallCheck(this, Dendrite);

    this.type = 'dendrite';

    var rColor = settings.rColor,
...
var Node = function () {
  ...
  _createClass(Node, [{
    ...
    key: 'update',
    value: function update(nodeArray, electronArray) {
      var _this2 = this;
      ...
      electronArray.forEach(function (electron) {
        if (electron.type === 'electron-standard') {
          var distX = Math.abs(electron.x - _this2.x);
          var distY = Math.abs(electron.y - _this2.y);

          var dist = Math.sqrt(distX * distX + distY * distY);

          if (dist < 120) {
            // close enough to shrink
            _this2.shrinking = dist;
          }
        }
        /*
    ...
    key: 'draw',
    value: function draw(context, settings) {
      var ctx = context;
      var rColor = settings.rColor;
      var sColor = settings.sColor;
      var colorFadeTime = settings.electronStandardSettings.colorFadeTime;
      var redFadeInTime = settings.electronStandardSettings.redFadeInTime;
...
var Controller = function () {
  function Controller(context, canvas, img) {
  ...
  _createClass(Controller, [{
    key: 'beginResizeCanvasToWindow',
    value: function beginResizeCanvasToWindow() {
      var _this3 = this;

      this.canvas.width = window.innerWidth;
      this.canvas.height = window.innerHeight;
      window.addEventListener('resize', function () {
        if (window.innerWidth !== _this3.canvas.width) {
    ...
    key: 'extractDataFromHandImage',
    value: function extractDataFromHandImage() {
      if (this.imageLoaded === false) {
        ...
        this.hand = this.offScreenHandCanvasContext.getImageData(0, 0, // sx, sy
        this.offScreenHandCanvas.width, this.offScreenHandCanvas.height // sw, sh
        );

        for (var i = 0; i < this.hand.data.length; i += 4) {
    ...
    key: 'scaleAndRenderToOnScreenCanvas',
    value: function scaleAndRenderToOnScreenCanvas() {
      var apsectRatio = this.offScreenNodeCanvas.width / this.offScreenNodeCanvas.height;

      var height = this.canvas.height;

      var width = height * apsectRatio;

      var xOffset = this.canvas.width / 2 - width / 2;
      var yOffset = this.canvas.height / 2 - height / 2;

      this.ctx.drawImage(this.offScreenNodeCanvas, xOffset, yOffset, width, height);
    }
  }, {
    key: 'createHandOutOfNodes',
    value: function createHandOutOfNodes(settings) {
      this.nodeArray = [];
      ...
      var paddingW = this.handImage.width / padScale;
      var paddingH = this.handImage.height / padScale;

      var totalPaddingW = paddingW * 2;
      var totalPaddingH = paddingH * 2;
      ...
      for (var w = 16; w < this.handImage.width - 16; w += 8) {
        for (var h = 16; h < this.handImage.height - 16; h += 8) {
          if (getPixelBrightness(this.hand, w, h) < 100 && Math.random() > 0.4) {
            // place a node
            var x = w + paddingW;
            var y = h + paddingH;

            var _settings$nodeSetting = settings.nodeSettings,
                minRadius = _settings$nodeSetting.minRadius,
                maxRadius = _settings$nodeSetting.maxRadius;
    ...
    key: 'makeAnElectron',
    value: function makeAnElectron(settings, type) {
      var x = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 0;
      var y = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 0;

      this.electronArray.push(new Electron(settings, this.nodeArray, type, x, y, this.canvas, this.offScreenNodeCanvas));
    }
  }, {
    key: 'makeADendrite',
    value: function makeADendrite(settings, x0, y0, x1, y1) {
      this.dendriteArray.push(new Dendrite(settings, x0, y0, x1, y1));
    }
  }, {
    key: 'makeDendrites',
    value: function makeDendrites(settings) {
      var _this4 = this;

      this.dendriteArray = [];

      var _settings$dendriteSet = settings.dendriteSettings,
          reachDistance = _settings$dendriteSet.reachDistance,
          density = _settings$dendriteSet.density;


      this.nodeArray.forEach(function (node0) {
        _this4.nodeArray.forEach(function (node1) {
          if (reachableViaDendrite(node0.x, node0.y, node1.x, node1.y, reachDistance) && Math.random() > 1 - density) {
            _this4.makeADendrite(settings, node0.x, node0.y, node1.x, node1.y);
    ...
    key: 'beginElectronSpawnOnClick',
    value: function beginElectronSpawnOnClick(settings) {
      var _this5 = this;
      ...
      window.addEventListener('click', function (e) {
        ...
        if (_this5.canvas.height > mouseY) {
          _this5.makeAnElectron(settings, 'electron-supercharged', mouseX, mouseY);
        }
      }, false);
    }
  }, {
    key: 'writeMyNameOnScreen',
    value: function writeMyNameOnScreen() {
      var textSize = this.canvas.width / 30;
    ...
    key: 'electronSpawnRepeat',
    value: function electronSpawnRepeat(settings) {
      var _this6 = this;

      setInterval(function () {
        return _this6.makeAnElectron(settings, 'electron-standard');
      }, settings.electronStandardSettings.timeSpread);
    }
  }, {
    key: 'superchargedElectronSpawnRepeat',
    value: function superchargedElectronSpawnRepeat(settings) {
      var _this7 = this;

      setInterval(function () {
        return _this7.makeAnElectron(settings, 'electron-supercharged', getRandomInt(_this7.canvas.width), getRandomInt(_this7.canvas.height));
      }, settings.randomSuperchargedTimeSpread);
    }
  }, {
    key: 'removeElectron',
    value: function removeElectron(electron) {
      var index = this.electronArray.indexOf(electron);

      var doesExist = index > -1;
      if (doesExist) {
    ...
    key: 'updateElectrons',
    value: function updateElectrons() {
      var _this8 = this;

      this.electronArray().forEach(function (electron) {
        var status = electron.update(_this8.electronArray, _this8.nodeArray);
        if (status === 'dead') {
          _this8.removeRogueelectron();
        }
      });
    }
  }, {
    key: 'whatToAnimate',
    value: function whatToAnimate() {
      return [].concat(_toConsumableArray(this.dendriteArray), _toConsumableArray(this.nodeArray), _toConsumableArray(this.electronArray));
    }
  }, {
    key: 'animate',
    value: function animate(settings) {
      var _this9 = this;

      this.ctx.clearRect(0, 0, this.canvas.width, this.canvas.height);
      this.offScreenNodeCanvasContext.clearRect(0, 0, this.offScreenNodeCanvas.width, this.offScreenNodeCanvas.height);

      var objectsToAnimate = this.whatToAnimate();

      objectsToAnimate.forEach(function (object) {
        switch (object.type) {
          case 'dendrite':
            {
              object.draw(_this9.offScreenNodeCanvasContext);
              break;
            }
          case 'node':
            {
              object.update(_this9.nodeArray, _this9.electronArray);
              object.draw(_this9.offScreenNodeCanvasContext, settings, _this9.nodeArray);
              break;
            }
          case 'electron-standard':
            {
              var lifeStatus = object.update(_this9.canvas, _this9.offScreenNodeCanvas);
              if (lifeStatus === 'dead') {
                _this9.removeElectron(object);
              } else {
          ...
          case 'electron-supercharged':
            {
              var _lifeStatus = object.update(_this9.canvas, _this9.offScreenNodeCanvas);
              if (_lifeStatus === 'dead') {
                _this9.removeElectron(object);
              } else {
                // object.draw(this.ctx);
                object.drawSuperchargedTrial(_this9.ctx, settings.rColor);
              }
              break;
            }
          default:
